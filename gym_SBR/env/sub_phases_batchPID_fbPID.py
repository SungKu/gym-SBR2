...
class filling(object):
    ...
    def sim_rxn(self,t_start, t_end,t_delta, x, Spar,Kpar,DO_control_par,loading, kla_memory, u_batch):
        ...
        """ Feedback Controller : Conventional PID """
        ...
        # parameters of C-PID controller
        Kc = DO_control_par[0]
        tauI = DO_control_par[1]
        tauD = DO_control_par[9]

        Kla_memory.append(kla_memory[0])


        """ Feed-forward Controller : Conventional PID """
        ...
        for i in range(len(t_save2)-1) :
            ...
            for ii in range(len(t_range)-1):
                ...
                Kla_memory_instant = u_batch[9*i+ii+1] + kla_memory[9*i+ii+1]

                if Kla_memory_instant > DO_control_par[5]:  # check upper limit
                    Kla_memory_instant = DO_control_par[5]
                if Kla_memory_instant < DO_control_par[4]:  # check lower limit
                    Kla_memory_instant = DO_control_par[4]

                Kla_memory.append(Kla_memory_instant)
        ...
        for i in range(len(t_save2)-1) :
            ...
            # INPUT of feedback and feed-forward
            KLA =Kla[i] + Kla_memory[9 * i + 1]


            soln = integrate.odeint(self.dxdt, x, t_range,
                                    args=(Spar, Kpar, DO_control_par, KLA, loading,))
        ...
        return  t_memory, x_memory, AE, ME, Kla_memory, sp_memory, So_memory
...
class rxn(object):
    ...
    def sim_rxn(self, t_start, t_end,t_delta, x, Spar, Kpar, DO_control_par, kla_memory, u_batch):
        ...
        for i in range(len(t_save2)-1) :
            ...
            soln = integrate.odeint(self.dxdt, x, t_range,
                                    args=(Spar, Kpar, DO_control_par, KLA,))  # ,full_output=0)#,rtol=1.49012e-2)
