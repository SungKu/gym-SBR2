...
action_space = spaces.Box(np.array([-0.05]), np.array([0.05]),
...
"""
state_idx = [0, 1, 5, 6, 8, 9, 10]
x_1 = np.array([0.5, 30, 2599., 168., 2., 13., 0.005])
x_2 = np.zeros([1, len(state_idx)])
...
# Filling phase
x_out, t_range, Kla, So, dcv, ie, e = Sim_filling(x_in, t, t_ratio[0], influent_mixed, Kla, So, dcv, ie, e)
...
for i in state_idx :
    if i == 0:
        ...
        x_2[0][state_idx.index(i)] = (Qin * influent_mixed[i] + x_out_1[i] * IV) / (Qin + IV)
...
t, x_out = run_step(t, u, x_in, influent_mixed, done)
...
for i in state_idx :
    if i == 0:
        ...
        x_2[0][state_idx.index(i)] = x_in[i]
state = x_2 / x_1
...
if t >= t_memory5[-1]:
    ...
    x_out1, t_range1, Qw, PE, SP, EQI, eff_component = Sim_Settling_Drawing(x_in, t, t_ratio[5],
                                                                                 t_ratio[6], t_delta, Qeff,
                                                                                 biomass_setpoint)
    x_in = x_out1[-1]
    x_out2, t_range2, Kla, So, dcv, ie, e = Sim_idle(x_in, t_range1, t_ratio[7], u, Kla, So, dcv, ie,
                                                          e)
    ...
    u_t_t = np.vstack([u_t_t, u_t])



#return state, reward, done, {}

def run_step( t, u, x_in, influent_mixed, done):  # steps for rxn phases
    ...
    x_out, t_range, Kla, So, dcv, ie, e = Sim_rxn(x_in, t_range, u, Kla, So, dcv, ie, e)
...
def filling_dxdt(x, t, Spar, Kpar, DO_control_par, Kla, loading):
    # Process 1
...
def Sim_filling(x, t, t_filling, loading, Kla, So, dcv, ie, e):
    t_start = t
    ...
    x_out = integrate.odeint(filling_dxdt, x, t_range,
                             args=(Spar, Kpar, DO_control_par, Kla[-1], loading,))
...
def reaction_dxdt( x, t, Spar, Kpar, DO_control_par, Kla):
    # Process 1
    rho1 = Kpar[0] * (x[2] / (Kpar[1] + x[2])) * (x[8] / (Kpar[2] + x[8])) * x[5]
...
def Sim_rxn(x, t_range, u, Kla, So, dcv, ie, e):
    ...
    x_out = integrate.odeint(reaction_dxdt, x, t_range,
                             args=(Spar, Kpar, DO_control_par, Kla[-1],))
...
def settling_dXnddt(Xnd, t, z, Xf):
...
def Sim_Settling_Drawing(x, t, t_settling, t_drawing, dt, Qeff, biomass_setpoint):
    # Settling phases
    ...
    dXnddt = integrate.odeint(settling_dXnddt, Xnd, t_range_settling, args=(z, Xf,))
    dsXdt = integrate.odeint(settling_dsXdt, sX, t_range_settling, args=(z, Xf,))
    ...
    EQI, eff_component = cal_eq(sX_eff, X_eff, waste_sX_weight, Qw)
...
def idle_dxdt(x, t, Spar, Kpar, DO_control_par, Kla):
    # Process 1
    rho1 = Kpar[0] * (x[2] / (Kpar[1] + x[2])) * (x[8] / (Kpar[2] + x[8])) * x[5]
...
def Sim_idle(x, t_range, t_idle, u, Kla, So, dcv, ie, e):
    ...
    x_out = integrate.odeint(idle_dxdt, x, t_range_idle,
                             args=(Spar, Kpar, DO_control_par, Kla[-1],))
...
def cal_eq(sX_eff, x_eff, waste_sX_weight, Qw):
